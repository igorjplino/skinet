...
export class CheckoutPaymentComponent implements OnInit {
  ...
  cardExpireComplete = false;
  cardCvcComplete = false;
  cardErrors: any;
  loading = false;

  constructor(
    private basketSrvice: BasketService,
    private checkoutService: CheckoutService,
    private toastr: ToastrService,
    private router: Router) { }

  ngOnInit(): void {
    loadStripe("pk_test_51OvhmV2KAM6YfI3kD8lO5aSm4K4U5Zthbdd5nTwG3xb4xbVryIsqtzO8aGyVpTzev2z8t8uKKnvcf3277oJmL20A00wr3R36bU")
      .then(stripe => {
        this.strip = stripe;
        ...
        if (elements) {
          ...
          this.cardExpiry.on("change", event => {
            this.cardExpireComplete = event.complete;
            if (event.error) {
              this.cardErrors = event.error.message;
            }
            else {
  ...
  get paymentFormComplete() {
    return this.checkoutForm?.get("paymentForm")?.valid &&
      ...
      this.cardExpireComplete &&
      this.cardCvcComplete;
  }

  async submitOrder() {
    this.loading = true;
    const basket = this.basketSrvice.getCurrentBasketValue();
    ...
    try {
      ...
      if (paymentResult.paymentIntent) {
        this.basketSrvice.deleteBasket(basket!);
